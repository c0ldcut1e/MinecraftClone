...
#else
        [[nodiscard]] constexpr friend bool operator==(const version &lhs, const version &rhs) noexcept { return std::tuple(lhs.major, lhs.minor, lhs.patch) == std::tuple(rhs.major, rhs.minor, rhs.patch); }

        [[nodiscard]] constexpr friend bool operator!=(const version &lhs, const version &rhs) noexcept { return !(lhs == rhs); }
...
#define BS_THREAD_POOL_DEFINE_BITWISE_OPERATOR(ENUM, OP)                                                                                                                                                                                                           \
    constexpr ENUM operator OP(const ENUM lhs, const ENUM rhs) noexcept { return static_cast<ENUM>(static_cast<std::underlying_type_t<ENUM>>(lhs) OP static_cast<std::underlying_type_t<ENUM>>(rhs)); }                                                            \
    constexpr ENUM &operator OP## = (ENUM & lhs, const ENUM rhs) noexcept { return lhs = lhs OP rhs; }
    ...
    constexpr tp operator~(const tp value) noexcept { return static_cast<tp>(~static_cast<std::underlying_type_t<tp>>(value)); }
 ...
 */
     ...
     */
        [[nodiscard]] friend bool operator<(const pr_task &lhs, const pr_task &rhs) noexcept { return lhs.priority < rhs.priority; }
     ...
     */
        ...
        bool wait_until(const std::chrono::time_point<C, D> &timeout_time) const {
            for (const std::future<T> &future : *this) {
                ...
                if (timeout_time < C::now()) return false;
            }
            return true;
        }
    }; // class multi_future
 ...
 */
     ...
     */
        [[nodiscard]] T end(const std::size_t block) const noexcept { return (block == num_blocks - 1) ? index_after_last : start(block + 1); }
     ...
     */
        [[nodiscard]] T start(const std::size_t block) const noexcept { return first_index + static_cast<T>(block * block_size) + static_cast<T>(block < remainder ? block : remainder); }
 ...
 */
    enum class os_process_priority { idle = IDLE_PRIORITY_CLASS, below_normal = BELOW_NORMAL_PRIORITY_CLASS, normal = NORMAL_PRIORITY_CLASS, above_normal = ABOVE_NORMAL_PRIORITY_CLASS, high = HIGH_PRIORITY_CLASS, realtime = REALTIME_PRIORITY_CLASS };
 ...
 * @brief An enum containing pre-defined OS-specific process priority values for portability.
 */
    enum class os_process_priority { idle = PRIO_MAX - 2, below_normal = PRIO_MAX / 2, normal = 0, above_normal = PRIO_MIN / 3, high = PRIO_MIN * 2 / 3, realtime = PRIO_MIN };
...
#if defined(_WIN32)
        DWORD_PTR process_mask = 0;
        for (std::size_t i = 0; i < std::min<std::size_t>(affinity.size(), sizeof(DWORD_PTR) * 8); ++i) process_mask |= (affinity[i] ? (1ULL << i) : 0ULL);
 ...
 */
     ...
     */
        static bool set_os_thread_affinity([[maybe_unused]] const std::vector<bool> &affinity) {
            ...
            for (std::size_t i = 0; i < std::min<std::size_t>(affinity.size(), sizeof(DWORD_PTR) * 8); ++i) thread_mask |= (affinity[i] ? (1ULL << i) : 0ULL);
     ...
     */
        [[nodiscard]] static std::optional<os_thread_priority> get_os_thread_priority() {
            ...
            if (policy == SCHED_OTHER) {
                ...
                if (param.sched_priority == sched_get_priority_max(SCHED_OTHER)) return os_thread_priority::above_normal;
                if (param.sched_priority == sched_get_priority_min(SCHED_OTHER) + (sched_get_priority_max(SCHED_OTHER) - sched_get_priority_min(SCHED_OTHER)) / 2) return os_thread_priority::normal;
                if (param.sched_priority == sched_get_priority_min(SCHED_OTHER) + (sched_get_priority_max(SCHED_OTHER) - sched_get_priority_min(SCHED_OTHER)) * 2 / 3) return os_thread_priority::below_normal;
                if (param.sched_priority == sched_get_priority_min(SCHED_OTHER) + (sched_get_priority_max(SCHED_OTHER) - sched_get_priority_min(SCHED_OTHER)) / 3) return os_thread_priority::lowest;
     ...
     */
        static bool set_os_thread_priority(const os_thread_priority priority) {
            ...
            if (nice_val.has_value()) success = success && (setpriority(PRIO_PROCESS, static_cast<id_t>(syscall(SYS_gettid)), nice_val.value()) == 0);
            return success;
#elif defined(__APPLE__)
 ...
 */
        ...
        void detach_bulk(const I first, const I last, const priority_t priority = 0) {
            if (first != last) {
                bool notify = false;
                {
                    ...
                    if constexpr (pause_enabled) notify = tasks.empty() && !paused;
                    else
                        notify = tasks.empty();
                    for (I it = first; it != last; ++it) {
                        if constexpr (priority_enabled) tasks.emplace(std::move(*it), priority);
                        else
                            tasks.emplace(std::move(*it));
                    }
                }
                if (notify) task_available_cv.notify_all();
            }
        }
     ...
     */
        ...
        void detach_task(F &&task, const priority_t priority = 0) {
            {
                const std::scoped_lock tasks_lock(tasks_mutex);
                if constexpr (priority_enabled) tasks.emplace(std::forward<F>(task), priority);
                else
     ...
     */
        ...
        void reset(const std::size_t num_threads, F &&init) {
            if constexpr (pause_enabled) {
                ...
                paused = was_paused;
                tasks_lock.unlock();
                if (!was_paused) task_available_cv.notify_all();
            } else {
                reset_pool(num_threads, std::forward<F>(init));
            }
        }
     ...
     */
        void wait() {
            ...
            tasks_done_cv.wait(tasks_lock, [this] {
                if constexpr (pause_enabled) return (tasks_running == 0) && (paused || tasks.empty());
                else
     ...
     */
        ...
        [[nodiscard]] N enqueue_blocks(const T first_index, const T index_after_last, F &&block, std::size_t num_blocks, const priority_t priority = 0) {
            if (index_after_last > first_index) {
                ...
                for (std::size_t i = 0; i < num_blocks; ++i) all_tasks.emplace_back(block_task_t{block_ptr, blks.start(i), blks.end(i)});
                if constexpr (submit) return submit_bulk(all_tasks, priority);
                else
                    detach_bulk(all_tasks, priority);
            }
            return N();
     ...
     */
        ...
        [[nodiscard]] N enqueue_loop(const T first_index, const T index_after_last, F &&loop, std::size_t num_blocks, const priority_t priority = 0) {
            if (index_after_last > first_index) {
                ...
                all_tasks.reserve(num_blocks);
                for (std::size_t i = 0; i < num_blocks; ++i) all_tasks.emplace_back(loop_task_t{loop_ptr, blks.start(i), blks.end(i)});
                if constexpr (submit) return submit_bulk(all_tasks, priority);
                else
                    detach_bulk(all_tasks, priority);
            }
     ...
     */
        ...
        [[nodiscard]] N enqueue_sequence(const T first_index, const T index_after_last, F &&sequence, const priority_t priority = 0) {
            if (index_after_last > first_index) {
                ...
                for (T i = first_index; i < index_after_last; ++i) all_tasks.emplace_back(sequence_task_t{sequence_ptr, i});
                if constexpr (submit) return submit_bulk(all_tasks, priority);
                else
                    detach_bulk(all_tasks, priority);
            }
     ...
     */
        [[nodiscard]] task_t pop_task() {
            task_t task;
            if constexpr (priority_enabled) task = std::move(tasks.top().task);
            else
        ...
        void worker(BS_THREAD_POOL_WORKER_TOKEN const std::size_t idx) {
            ...
            while (true) {
                ...
                task_available_cv.wait(tasks_lock BS_THREAD_POOL_WAIT_TOKEN, [this] {
                    if constexpr (pause_enabled) return !(paused || tasks.empty()) BS_THREAD_POOL_OR_STOP_CONDITION;
                    else
