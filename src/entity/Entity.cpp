...
Vec3 Entity::getRenderPosition(float partialTicks) const
{
    ...
    return Vec3(a.x + (b.x - a.x) * partialTicks, a.y + (b.y - a.y) * partialTicks,
                a.z + (b.z - a.z) * partialTicks);
}
...
float Entity::getRenderPitch(float partialTicks) const
{
    return m_oldPitch + (m_pitch - m_oldPitch) * partialTicks;
}
...
void Entity::applyPhysics(double deltaTime)
{
    ...
    if (m_flying)
    {
        m_velocity = wishDirection.scale(m_maxSpeed);
    }
    else
    {
    ...
    if (m_onGround && m_velocity.y < 0.0)
    {
        m_velocity.y = 0.0;
    }
