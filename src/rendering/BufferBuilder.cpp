...
void BufferBuilder::begin(uint32_t mode)
{
    m_mode = mode;
    m_vertices.clear();
}

void BufferBuilder::color(uint32_t argb)
{
...
void BufferBuilder::vertex(float x, float y, float z) { vertexUV(x, y, z, 0.0f, 0.0f); }
...
void BufferBuilder::bindTexture(Texture *texture) { m_texture = texture; }
...
void BufferBuilder::setScreenProjection(const Mat4 &projection) { m_screenProjection = projection; }
...
void BufferBuilder::end()
{
    ...
    RenderCommand::uploadArrayBuffer(
            m_vertices.data(), (uint32_t) (m_vertices.size() * sizeof(Vertex)), GL_DYNAMIC_DRAW);
