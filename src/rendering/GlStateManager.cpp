...
void GlStateManager::setStencilFunc(uint32_t func, int ref, uint32_t mask)
{
    RenderCommand::setStencilFunc(func, ref, mask);
}

void GlStateManager::setStencilOp(uint32_t sfail, uint32_t dpfail, uint32_t dppass)
{
...
void GlStateManager::setScissor(int x, int y, int w, int h)
{
...
{
    if (s_colorMaskR == r && s_colorMaskG == g && s_colorMaskB == b && s_colorMaskA == a)
    {
        return;
    }
...
{
    if (s_polygonOffsetFactor == factor && s_polygonOffsetUnits == units)
    {
        return;
    }
...
{
    if (s_polygonFace == face && s_polygonMode == mode)
    {
        return;
    }
...
{
    s_matrix = s_matrix.multiply(gsmTranslation((double) x, (double) y, (double) z));
}

void GlStateManager::scalef(float x, float y, float z)
{
    s_matrix = s_matrix.multiply(gsmScale((double) x, (double) y, (double) z));
}
