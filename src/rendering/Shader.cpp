...
Shader::Shader(const char *vertexPath, const char *fragmentPath)
{
    ...
    if (!RenderCommand::getProgramLinkStatus(m_program))
    {
...
{
    RenderCommand::setUniform1i(getLocation(name), value);
}

void Shader::setFloat(const char *name, float value) const
{
    RenderCommand::setUniform1f(getLocation(name), value);
}

void Shader::setVec2(const char *name, float x, float y) const
{
    RenderCommand::setUniform2f(getLocation(name), x, y);
}
...
uint32_t Shader::compileShader(uint32_t type, const char *src)
{
    ...
    if (!RenderCommand::getShaderCompileStatus(shader))
    {
