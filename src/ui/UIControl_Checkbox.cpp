...
static constexpr float CHECKBOX_VIRTUAL_SIZE  = 28.0f;
static constexpr float CHECKBOX_LABEL_SCALE   = 0.8f;
static constexpr float CHECKBOX_LABEL_SPACING = 4.0f;
...
void UIControl_Checkbox::render()
{
    ...
    float textBaselineY    = std::round(boxY + boxSize - lineBottomOffset);
    font->setNearest(true);
    if (m_selected)
    {
        float shadowOffset = std::max(1.0f, std::round(uiScale * 0.5f));
        font->draw(m_label, textX + shadowOffset, textBaselineY + shadowOffset, fontScale,
                   0xFF000000);
    }
