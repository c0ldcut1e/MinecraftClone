...
#define SLIDER_DEFAULT_WIDTH          550.0f
#define SLIDER_DEFAULT_HEIGHT         40.0f
#define SLIDER_TEXT_SCALE             0.9f
#define SLIDER_SELECTED_TEXT_COLOR    0xFFFFFF00
#define SLIDER_DEFAULT_TEXT_COLOR     0xFFFFFFFF
#define SLIDER_SELECTED_OUTLINE_COLOR 0xFFFFFF00
#define SLIDER_SOURCE_BORDER_PIXELS   2.0f
...
UIControl_Slider::UIControl_Slider(float x, float y, float width, float height,
    ...
    : UIControl_Slider(x, y, width, height, name, minValue, maxValue, defaultValue, increment,
                       "textures/ui/slider_background.png",
                       "textures/ui/slider_background_highlight.png",
                       "textures/ui/slider_graphic.png")
{}
...
void UIControl_Slider::render()
{
    ...
    if (!m_text.empty())
    {
        ...
        font->draw(m_text, textX + 1.0f, textBaselineY + 1.0f, fontScale, 0xFF000000);
...
bool UIControl_Slider::setValue(float value)
{
    ...
    if (fabs(m_value - clampedValue) < 0.0001f)
    {
        return false;
    }
...
UIControl_Slider::UIControl_Slider(float x, float y, float width, float height,
                                   ...
                                   const char *highlightTexturePath, const char *graphicTexturePath)
    : UIControl("Control_Slider", x, y, width, height), m_name(name), m_text(name), m_value(0.0f),
      ...
      m_backgroundTexture(backgroundTexturePath), m_highlightTexture(highlightTexturePath),
      m_graphicTexture(graphicTexturePath)
{
    if (fabsf(m_maxValue - m_minValue) < 0.0001f)
    {
