...
FixedStepper::FixedStepper(double stepSeconds, double maxFrameSeconds)
    : m_step(stepSeconds), m_maxFrame(maxFrameSeconds), m_accumulator(0.0)
{}

void FixedStepper::addFrame(double deltaSeconds)
{
    if (deltaSeconds > m_maxFrame)
    {
        deltaSeconds = m_maxFrame;
    }
    if (deltaSeconds < 0.0)
    {
        deltaSeconds = 0.0;
