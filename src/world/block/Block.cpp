...
void Block::setTintColormap(Direction *direction, const std::string &colormapName)
{
    m_tintColormaps[direction] = colormapName;
}

bool Block::hasTintColormap(Direction *direction) const
{
    return m_tintColormaps.find(direction) != m_tintColormaps.end();
}
                            ...
                            int localZ) const
{
    ...
    float g;
    float b;
    ColormapManager::getInstance()->sampleFoliageColor(getTintColormap(direction),
                                                       totalTemp / (float) samples,
                                                       totalHumid / (float) samples, &r, &g, &b);
...
{
    if (m_hasInteractionAabb)
    {
        return m_interactionAabb;
    }
    return m_aabb;
}
...
AABB Block::getPlacedAABB(const BlockPos &pos, Direction *attachmentFace) const
{
    ...
    if (!m_hasWallMountedTransform)
    {
        return box;
    }
    if (attachmentFace == Direction::NORTH)
    {
        return box.translated(
                Vec3(0.0, 0.0, -(m_wallMountedInset + m_interactionAttachmentOffset)));
    }
    if (attachmentFace == Direction::SOUTH)
    {
        return box.translated(Vec3(0.0, 0.0, m_wallMountedInset + m_interactionAttachmentOffset));
    }
    if (attachmentFace == Direction::WEST)
    {
        return box.translated(
                Vec3(-(m_wallMountedInset + m_interactionAttachmentOffset), 0.0, 0.0));
    }
    if (attachmentFace == Direction::EAST)
    {
        return box.translated(Vec3(m_wallMountedInset + m_interactionAttachmentOffset, 0.0, 0.0));
    }
    return box;
}
...
void Block::setUVRect(Direction *direction, float u0, float v0, float u1, float v1)
{
    m_uvRects[direction] = {u0, v0, u1, v1};
}
