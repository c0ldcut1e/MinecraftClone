...
CloudMesh::CloudMesh() : m_vao(0), m_vbo(0), m_vertexCount(0)
{
    ...
    RenderCommand::setVertexAttribPointer(1, 4, GL_FLOAT, false, 7 * sizeof(float),
                                          3 * sizeof(float));
}
...
void CloudMesh::upload(const std::vector<float> &vertices)
{
    ...
    RenderCommand::bindVertexArray(m_vao);
    RenderCommand::bindArrayBuffer(m_vbo);
    RenderCommand::uploadArrayBuffer(vertices.data(), (uint32_t) (vertices.size() * sizeof(float)),
                                     GL_DYNAMIC_DRAW);
}
